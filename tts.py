...
async def send_tts_request(
    ...
    payload = {
        "input": prompt,
        "task_type": "Base",
        "ref_audio": "file:///home/x50058110/tts-aclg/sample.wav",
        "ref_text": "增速提高三点四个百分点",
        "voice": voice,
        "language": language,
        "stream": True,
        "response_format": "pcm",
    ...
    try:
        async with session.post(api_url, json=payload) as response:
            ...
            if audio_dir is not None:
                ...
                safe_id = request_id if request_id is not None else int(st * 1e9)
                wav_path = audio_dir / f"req_{safe_id}.wav"
            ...
            async for chunk in response.content.iter_any():
                ...
                if wav_file is not None and len(chunk) > 0:
                    wav_file.writeframesraw(chunk)

            result.e2e = time.perf_counter() - st
            result.audio_bytes = total_bytes
            result.audio_duration = pcm_bytes_to_duration(total_bytes, sample_rate=sample_rate, sample_width=sample_width)
...
async def run_benchmark(
    ...
    async def limited_request(req_id: int, prompt: str):
        async with semaphore:
            req_audio_dir = None if audio_dir is None else (audio_dir / f"concurrency_{max_concurrency}")
            return await send_tts_request(
                ...
                language,
                request_id=req_id,
                audio_dir=req_audio_dir,
                pbar=pbar,
            )

    start_time = time.perf_counter()
    tasks = [asyncio.create_task(limited_request(i, p)) for i, p in enumerate(request_prompts)]
    ...
    print(f"{'Std E2EL (ms):':<40}{bench.std_e2e_ms:<10.2f}")
    ...
    print(f"{'Std AUDIO_TTFP (ms):':<40}{bench.std_ttfp_ms:<10.2f}")
    ...
    print(f"{'Std AUDIO_RTF:':<40}{bench.std_rtf:<10.3f}")
...
async def main(args):
    ...
    audio_dir = None
    if args.audio_dir:
        audio_dir = Path(args.audio_dir) / f"{args.config_name}_{timestamp}"
...
def parse_args():
    ...
    parser.add_argument(
        "--audio-dir",
        type=str,
        default="results_audio",
        help="Directory to save generated audio WAV files (empty disables saving)",
    )
